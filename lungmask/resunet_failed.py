...
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, depth=5, wf=6, padding=False,
        ...
        sequence = []
        sequence.append(nn.ConvTranspose2d(prev_channels, 2 ** (wf - 1), kernel_size=1,
                                         stride=2, output_padding=1))
        sequence.append(nn.Conv2d(2 ** (wf - 1), out_channels, kernel_size=1))
        self.last = nn.Sequential(*sequence)
    ...
    def __init__(self, in_size, out_size, up_mode, padding, batch_norm, residual=False):
        ...
        if up_mode == 'upconv':
            self.up = nn.ConvTranspose2d(in_size, out_size, kernel_size=3,
                                         stride=2, output_padding=1, dilation=2)
